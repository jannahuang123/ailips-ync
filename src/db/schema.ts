...
export const users = pgTable(
  "users",
  {
    ...
    subscription_tier: varchar({ length: 50 }).notNull().default("free"),
    total_videos_created: integer().notNull().default(0),
    last_video_created_at: timestamp({ withTimezone: true }),
  },
  (table) => [
...
export const projects = pgTable("projects", {
  ...
  result_url: varchar({ length: 500 }),
  external_task_id: varchar({ length: 255 }), // HeyGen/D-ID task ID
  provider: varchar({ length: 50 }).notNull().default("heygen"), // heygen, did
  quality: varchar({ length: 20 }).notNull().default("medium"), // low, medium, high, ultra
...
export const lipsyncTasks = pgTable("lipsync_tasks", {
  id: integer().primaryKey().generatedAlwaysAsIdentity(),
  project_uuid: varchar({ length: 255 }).notNull(),
  status: varchar({ length: 50 }).notNull().default("queued"), // queued, processing, completed, failed
